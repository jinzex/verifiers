...
def test_run_bash_isolates_apptainer_command(monkeypatch) -> None:
    ...
    assert program.run_bash("printf success", "~/.local/bin/tini") == "success"
    assert captured[0][0] == [
        "unshare",
        "--pid",
        "--fork",
...
outputs = [
    run_bash("! cat /proc/[0-9]*/comm | grep -q '^python'", {TINI_PATH!r}),
    run_bash("printf controller-survived", {TINI_PATH!r}),
    run_bash("for i in $(seq 1 128); do sh -c '(exit 0) &' ; done; sleep 1; "
             "for stat in /proc/[0-9]*/stat; do test $(cut -d' ' -f3 $stat) != Z || exit 1; done", {TINI_PATH!r}),
]
...
"""
        ...
        assert outputs == ["", "controller-survived", ""]
    finally:
        await runtime.stop()
